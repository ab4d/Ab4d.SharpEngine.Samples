...
namespace Ab4d.SharpEngine.Samples.WinUI.CameraControllers
{
    /// <summary>
...
PointerPosition: Zooms into the 3D position that is 'behind' current pointer position. If there is no 3D object behind pointer position, then camera is zoomed into the SceneView's center.");

            ToolTipService.SetToolTip(UsePointerPositionForMovementSpeedInfoControl,
@"When UsePointerPositionForMovementSpeed is true (CheckBox is checked) then the camera movement speed is determined by the distance to the 3D object behind the pointer. When no 3D object is behind the pointer or when UsePointerPositionForMovementSpeed is set to false, then movement speed is determined by the distance from the camera to the TargetPosition is used. Default value is true.");

            ToolTipService.SetToolTip(PointerWheelDistanceChangeFactorInfoControl,
@"PointerWheelDistanceChangeFactor specifies a value that is used when zooming with pointer wheel. When zooming out the Camera's Distance or CameraWidth is multiplied with this value. When zooming in the Camera's Distance or CameraWidth is divided with this value. Default value is 1.05. Bigger value increases the speed of zooming with mouse wheel.");

            ToolTipService.SetToolTip(MaxCameraDistanceInfoControl,
...
This property can be set to a reasonable number to prevent float imprecision when the camera distance is very big. Default value is float.NaN.");
            ...
            ToolTipService.SetToolTip(PointerMoveThresholdInfoControl,
@"This property specifies how much user needs to move the pointer before rotation, movement or quick zoom are started.}

Because PointerCameraController does not handle pointer events until pointer is moved for the specified amount, the events can be get by the user code (for example to handle pointer click; it is not needed to use Preview mouse events for that).

When 0 (by default), then rotation, movement or quick zoom are started immediately when the correct pointer buttons and keyboard modifiers are pressed (no pointer movement needed).");
