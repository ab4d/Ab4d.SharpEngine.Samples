...
//to the main CPU memory and then back to the application's GPU).";
            ...
            ToolTip.SetTip(MultisamplingComboBox, 
@"Multi-sampling anti-aliasing (MSAA) is the first anti-aliasing technique in Ab4d.SharpEngine.

MSAA produce smoother edges by storing multiple color values for each pixel. 
To improve performance, pixel shader is executed once for each pixel and then its result (color)
is shared across multiple pixels. This produces smoother edges but do not provide additional sub-pixel details.

For example, 4xMSAA runs pixel shader only once per each pixel but require 4 times the amount of memory.");

            ToolTip.SetTip(SuperSamplingComboBox,
@"Super-sampling anti-aliasing (SSAA) is the second anti-aliasing technique in Ab4d.SharpEngine.

SSAA is a technique that renders the image at a higher resolution and then down-samples the rendered image
to the final resolution. The Ab4d.SharpEngine can use smart down-sampling filter that improves smoothness
of the final image. Super-sampling produces smoother edges and also provides additional sub-pixel details.

Examples:
4xSSAA renders the scene to a texture with 4 times more pixels (width and height are multiplied by 2 - 
SceneView.SupersamplingFactor is 2). This requires running the pixel shader 4 times for each final pixel and requires 4 times the amount of memory.
...
In this case width and height are multiplied by 1.41 = sqrt(2) - SceneView.SupersamplingFactor is 1.41.");
        ...
        private void SetupDeviceInfo()
        {
            ...
            if (vulkanInstance == null)
            {
                ...
                {
                    MessageBox.Show(this, "Error creating VulkanInstance:\r\n" + ex.Message, "Error", MessageBox.MessageBoxButtons.Ok);
                    return;
                }
            }
            ...
            if (devicesCount > 1)
            {
                ...
                ToolTip.SetTip(GpuDevicesTextBlock,
@"When this samples project is started on laptop with multiple graphics cards,
then by default Windows will run the application by using the integrated graphics card.
...
to the main CPU memory and then back to the application's GPU).");
        ...
        private void SetupAntialisingComboBoxes(PhysicalDeviceDetails deviceDetails)
        {
            ...
            var sharpEngineSceneView = this.SharpEngineSceneView ?? new Ab4d.SharpEngine.AvaloniaUI.SharpEngineSceneView();
