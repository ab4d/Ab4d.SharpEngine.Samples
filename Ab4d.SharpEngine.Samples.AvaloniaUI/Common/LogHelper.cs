...
public static class LogHelper
{
    ...
    public static void SetupSharpEngineLogger(bool enableFullLogging, string? logFileName = null)
    {
        ...
        Log.RemoveAllLogListeners();

        if (enableFullLogging)
        {
            Log.LogLevel = LogLevels.Trace;
            Log.WriteSimplifiedLogMessage = false; // write full log messages timestamp, thread id and other details
        ...
        else
        {
            ...
            Log.LogLevel = LogLevels.Warn; // Log Warnings and Errors

            // Write log messages to output window (for example Visual Studio Debug window):
            if (logFileName == null)
            {
                Log.WriteSimplifiedLogMessage = true; // write log messages without timestamp, thread id and other details
                Log.IsLoggingToDebugOutput = true;
            }
            else
            {
