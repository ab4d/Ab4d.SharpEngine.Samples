...
{
    /// <summary>
    /// Interaction logic for AllObjectsTestScene.xaml
    /// </summary>
    public partial class AllObjectsTestScene : Page
    {
        ...
        private void CreateTestScene(Scene scene)
        {
            ...
            if (_bitmapIO != null && _bitmapIO.IsFileFormatImportSupported("jpg") && _bitmapIO.IsFileFormatImportSupported("png"))
            {
                ...
                {
                    Material = CreateTextureMaterial(@"Resources\10x10-texture.png", generateMipMaps: false),
                    //Material = CreateTextureMaterial(@"Resources\uvchecker2.jpg", generateMipMaps: false, isDeviceLocal: true),
                };

                texturedMaterialGroup.Add(geometryModel7);


                var textureMaterial = CreateTextureMaterial(@"Resources\uvchecker2.jpg", generateMipMaps: true);
                ...
                {
                    Material = CreateTextureMaterial(@"Resources\uvchecker.png"),
                };
            ...
            var customTexture = CreateCustomTexture(scene, 256, 128, alphaValue: 1);
            ...
            var treePlaneMaterial = CreateTextureMaterial(@"Resources\TreeTexture.png");
            ...
            //
            var solidColorMaterial2 = CreateTextureMaterial(@"Resources\10x10-texture.png", generateMipMaps: true);
            ...
            var solidColorMaterial3 = CreateTextureMaterial(@"Resources\10x10-texture.png", generateMipMaps: true);
            ...
            if (_bitmapIO != null)
            {
                ...
                string? fontFileName = fontPath + "arial_128.fnt"; // use arial font that is rendered with font size 128 pixels

                var arialBitmapFont = CreateBitmapFont(fontFileName, _bitmapIO);

                if (arialBitmapFont != null)
                {
                    var arialBitmapTextCreator = new BitmapTextCreator(scene, arialBitmapFont, _bitmapIO);
                    ...
                    var textNode1 = arialBitmapTextCreator.CreateTextNode(text: "Arial font",
                    ...
                    // Use ArialBlack font
                    fontFileName = fontPath + "arial_black_128.fnt";

                    var arialBlackBitmapFont = CreateBitmapFont(fontFileName, _bitmapIO);

                    if (arialBlackBitmapFont != null)
                    {
                        var arialBlackBitmapTextCreator = new BitmapTextCreator(scene, arialBlackBitmapFont, _bitmapIO);
                        _disposables.Add(arialBlackBitmapTextCreator);

                        var textNode3 = arialBlackBitmapTextCreator.CreateTextNode(text: "ArialBlack",
                    ...
                    fontFileName = fontPath + "arial_black_with_outline_128.fnt";

                    var arialBlackWithOutlineBitmapFont = CreateBitmapFont(fontFileName, _bitmapIO);

                    if (arialBlackWithOutlineBitmapFont != null)
                    {
                        var arialBlackWithOultineBitmapTextCreator = new BitmapTextCreator(scene, arialBlackWithOutlineBitmapFont, _bitmapIO);
                        ...
                        var textNode4 = arialBlackWithOultineBitmapTextCreator.CreateTextNode(text: "ArialBlack\r\nwith outline and\r\nSolidColorMaterial",
                                                                                              position: text1Position - new Vector3(0, textSize.Y - 20, 0),
        ...
        private void CreateLights()
        {
            ...
            _scene.Lights.Add(new PointLight(new Vector3(100, 0, 100), range: 10000) { Attenuation = new Vector3(1, 0, 0) });
        ...
        private void UpdateModelWorldMatrices()
        {
        ...
        private StandardMaterial CreateTextureMaterial(string imageFileName, CommonSamplerTypes samplerType = CommonSamplerTypes.Wrap, bool generateMipMaps = true, bool isDeviceLocal = true, float alphaClipThreshold = 0)
        {
            var gpuImage = CreateTexture(imageFileName, generateMipMaps, isDeviceLocal);

            if (gpuImage == null)
                return StandardMaterials.Black;

            var materialWithTexture = new StandardMaterial(gpuImage, samplerType, name: gpuImage.Name);
        ...
        private GpuImage? CreateTexture(string imageFileName, bool generateMipMaps = true, bool isDeviceLocal = true)
        {
            if (_bitmapIO == null || _scene == null)
                return null;

            return TextureLoader.CreateTexture(imageFileName, _bitmapIO, _scene.GpuDevice, generateMipMaps, isDeviceLocal);
        }
        ...
        public void LoadObject(string fileName)
        {
            ...
            if (!System.IO.File.Exists(fileName) && _bitmapIO != null && _bitmapIO.FileNotFoundResolver != null)
            {
                string? resolvedFileName = _bitmapIO.FileNotFoundResolver(fileName);
        ...
        private void OnIsAnimatingCheckBoxCheckedChanged(object sender, RoutedEventArgs e)
        {
            _isAnimatingScene = IsAnimatingCheckBox.IsChecked ?? false;
        }

        private void LoadRobotArmButton_OnClick(object sender, RoutedEventArgs e)
        {
            LoadObject("robotarm.obj");
        }

        private void LoadDragonButton_OnClick(object sender, RoutedEventArgs e)
        {
            LoadObject("dragon_vrip_res3.obj");
        }
    }
}
